...
startValdProc() {
  ...
  scalard vald-start ${TOFND_HOST:+--tofnd-host "$TOFND_HOST"} ${VALIDATOR_HOST:+--node "$VALIDATOR_HOST"} \
    --validator-addr "${VALIDATOR_ADDR:-$(scalard keys show validator -a --bech val)}" "$RECOVERY"
}
