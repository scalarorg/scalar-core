...
func checkBroadcaster(ctx context.Context, clientCtx client.Context, serverCtx *server.Context) error {
	...
	params := bankTypes.NewQueryBalanceRequest(broadcaster, scalarnet.NativeAsset)
	...
	if res.Balance.Amount.LTE(sdk.NewInt(minBalance)) {
		return fmt.Errorf("broadcaster does not have enough funds (minimum balance is %d%s)", minBalance, scalarnet.NativeAsset)
	}
