...
func (m RoutingContext) ToNexus() nexus.RoutingContext {
	return nexus.RoutingContext{Sender: m.Sender, FeeGranter: m.FeeGranter, Payload: m.Payload}
}

func RoutingContextFromNexus(ctx nexus.RoutingContext) RoutingContext {
	return RoutingContext{Sender: ctx.Sender, FeeGranter: ctx.FeeGranter, Payload: ctx.Payload}
}
...
func (m TransferState) ToNexus() nexus.TransferState {
	return nexus.TransferState(m)
}
...
func CrossChainAddressFromNexus(address nexus.CrossChainAddress) CrossChainAddress {
	return CrossChainAddress{Chain: ChainFromNexus(address.Chain), Address: address.Address}
}

func (m CrossChainAddress) ToNexus() nexus.CrossChainAddress {
	return nexus.CrossChainAddress{Chain: m.Chain.ToNexus(), Address: m.Address}
}
...
func TransferIDFromNexus(id nexus.TransferID) TransferID {
	return TransferID(id)
}

func (m TransferID) ToNexus() nexus.TransferID {
	return nexus.TransferID(m)
}
...
func (m Chain) ToNexus() nexus.Chain {
	return nexus.Chain{
		Name:                  m.Name.ToNexus(),
		SupportsForeignAssets: m.SupportsForeignAssets,
		KeyType:               m.KeyType,
		Module:                m.Module,
	}
}

func ChainFromNexus(chain nexus.Chain) Chain {
	return Chain{Name: ChainNameFromNexus(chain.Name), SupportsForeignAssets: chain.SupportsForeignAssets, KeyType: chain.KeyType, Module: chain.Module}
}
...
func AssetFromNexus(asset nexus.Asset) Asset {
	return Asset{Denom: asset.Denom, IsNativeAsset: asset.IsNativeAsset}
}

func (m Asset) ToNexus() nexus.Asset {
	return nexus.Asset{Denom: m.Denom, IsNativeAsset: m.IsNativeAsset}
}
...
func (c ChainName) ToNexus() nexus.ChainName {
	return nexus.ChainName(c)
}

func ChainNameFromNexus(chain nexus.ChainName) ChainName {
	return ChainName(chain)
}

func ChainNameFromString(chain string) ChainName {
	return ChainName(chain)
}
...
func (m TransferDirection) ToNexus() nexus.TransferDirection {
	return nexus.TransferDirection(m)
}
...
func (m GeneralMessage) ToNexus() nexus.GeneralMessage {
	return nexus.GeneralMessage{
		ID:          m.ID,
		Sender:      m.Sender.ToNexus(),
		Recipient:   m.Recipient.ToNexus(),
		PayloadHash: m.PayloadHash,
		Status:      m.Status.ToNexus(),
	}
}

func GeneralMessageFromNexus(msg nexus.GeneralMessage) GeneralMessage {
	return GeneralMessage{ID: msg.ID, Sender: CrossChainAddressFromNexus(msg.Sender), Recipient: CrossChainAddressFromNexus(msg.Recipient), PayloadHash: msg.PayloadHash, Status: GeneralMessage_Status(msg.Status)}
}

func (m GeneralMessage_Status) ToNexus() nexus.GeneralMessage_Status {
	return nexus.GeneralMessage_Status(m)
}
...
func (w WasmQueryRequest) ToNexus() nexus.WasmQueryRequest {
	return nexus.WasmQueryRequest{TxHashAndNonce: w.TxHashAndNonce, IsChainRegistered: &nexus.IsChainRegisteredRequest{Chain: w.IsChainRegistered.Chain}}
}
