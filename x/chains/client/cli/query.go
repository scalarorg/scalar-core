...
// getCurrentBlock returns the query to get the current block for the given chain
func getCurrentBlock() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "current-block [chain]",
		Short: "Returns the current block for the given chain",
		Args:  cobra.ExactArgs(1),
	}
	...
	cmd.RunE = func(cmd *cobra.Command, args []string) error {
		...
		res, err := queryClient.Block(cmd.Context(),
			&types.BlockRequest{
				Chain: args[0],
			})
		if err != nil {
