...
func getSupportedChains(ctx sdk.Context, n types.Nexus) []nexustypes.Chain {
	return slices.Filter(n.GetChains(ctx), types.IsSupportedChain)
}
...
func (q Querier) Chains(c context.Context, req *types.ChainsRequest) (*types.ChainsResponse, error) {
	...
	chains := getSupportedChains(ctx, q.nexus)
...
func getProof(key multisig.Key, signature multisig.MultiSig) ([]common.Address, []sdk.Uint, sdk.Uint, [][]byte) {
	...
	operators := slices.Map(participantsWithSigs, func(val sdk.ValAddress) types.Operator {
		...
		signature := funcs.Must(types.ToSignature(funcs.MustOk(signature.GetSignature(val)), common.BytesToHash(signature.GetPayloadHash()), pubKey))
