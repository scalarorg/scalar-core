...
func (r RateLimiter) RateLimitPacket(ctx sdk.Context, packet ibcexported.PacketI, direction common.TransferDirection, ibcPath string) error {
	...
	if err := r.nexus.RateLimitTransfer(ctx, chain.Name, token, direction.ToNexus()); err != nil {
		return err
	}
...
func (r RateLimitedICS4Wrapper) SendPacket(ctx sdk.Context, chanCap *capabilitytypes.Capability, packet ibcexported.PacketI) error {
	...
	return r.rateLimiter.RateLimitPacket(ctx, packet, common.TransferDirectionTo, types.NewIBCPath(packet.GetSourcePort(), packet.GetSourceChannel()))
}
