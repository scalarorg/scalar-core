...
// NewMsgServerImpl returns an implementation of the scalarnet MsgServiceServer interface for the provided Keeper.
...
func (s msgServer) Link(c context.Context, req *types.LinkRequest) (*types.LinkResponse, error) {
	...
	ctx.EventManager().EmitEvent(
		sdk.NewEvent(
			...
			sdk.NewAttribute(types.AttributeKeySourceChain, exported.Scalarnet.Name.String()),
	...
	s.Logger(ctx).Debug(fmt.Sprintf("successfully linked deposit %s on chain %s to recipient %s on chain %s for asset %s", depositAddress.String(), exported.Scalarnet.Name, req.RecipientAddr, req.RecipientChain, req.Asset),
		types.AttributeKeySourceChain, exported.Scalarnet.Name,
...
func (s msgServer) ConfirmDeposit(c context.Context, req *types.ConfirmDepositRequest) (*types.ConfirmDepositResponse, error) {
	...
	s.Logger(ctx).Info(fmt.Sprintf("deposit confirmed for %s on chain %s to recipient %s on chain %s for asset %s with transfer ID %d", req.DepositAddress.String(), exported.Scalarnet.Name, recipient.Address, recipient.Chain.Name, coin.String(), transferID),
	...
	for _, pendingTransfer := range pendingTransfers {
		success := utils.RunCached(ctx, s, func(ctx sdk.Context) (bool, error) {
			...
			if err = transfer(ctx, s.Keeper, s.nexus, s.bank, s.ibcK, recipient, pendingTransfer.Asset); err != nil {
				s.Logger(ctx).Error("failed to transfer asset to scalarnet", "err", err)
				return false, err
			}
