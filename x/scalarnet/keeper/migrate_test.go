...
func TestMigrate6to7(t *testing.T) {
	var (
		...
		scalarnetModuleAccAddr sdk.AccAddress
	)

	encCfg := app.MakeEncodingConfig()
	subspace := params.NewSubspace(encCfg.Codec, encCfg.Amino, sdk.NewKVStoreKey("scalarnetKey"), sdk.NewKVStoreKey("tscalarnetKey"), "scalarnet")
		...
		Then("the migration should lock back to escrow account and update sender of failed transfers", func(t *testing.T) {
			...
			for _, transfer := range transfers {
				...
				if transfer.Status == types.TransferFailed {
					assert.Equal(t, types.ScalarIBCAccount, actual.Sender)
				} else {
