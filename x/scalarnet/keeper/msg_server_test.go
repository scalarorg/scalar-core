...
func TestHandleMsgLink(t *testing.T) {
	...
	givenMsgServer := Given("an scalarnet msg server", func() {
		ctx, k, _, _ = setup()
	...
	t.Run("route IBC transfers", func(t *testing.T) {
		givenMsgServer.
			Branch(
				...
				When("dest chain is Scalarnet", func() {
					nexusK.GetChainFunc = func(sdk.Context, nexus.ChainName) (nexus.Chain, bool) {
						return exported.Scalarnet, true
					}
				}).
...
func TestRetryIBCTransfer(t *testing.T) {
	...
	givenMessageServer := Given("a message server", func() {
		...
		k.InitGenesis(ctx, types.DefaultGenesisState())
		cosmosChain := scalartestutils.RandomCosmosChain()
		chain = nexus.Chain{
...
func TestHandleCallContract(t *testing.T) {
	...
	givenMsgServer := Given("an scalarnet msg server", func() {
		...
		nexusK.GetChainByNativeAssetFunc = func(_ sdk.Context, asset string) (nexus.Chain, bool) { return exported.Scalarnet, true }
	})
			...
			Branch(
				whenChainIsRegistered.
					...
					Then("call contract succeeds", func(t *testing.T) {
						...
						assert.Equal(t, msg.GetSourceChain(), nexus.ChainName(exported.Scalarnet.Name))
