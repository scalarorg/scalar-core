...
// 			funcs.MustNoErr(k.RegisterAsset(ctx, evm.Ethereum, exported.NewAsset(scalarnet.NativeAsset, false), utils.MaxUint, time.Hour))
...
// 				assert.NoError(t, err)
// 				assert.Equal(t, response.Chains, []exported.ChainName{scalarnet.Scalarnet.Name, evm.Ethereum.Name, testChain.Name})
// 			}),
...
// 				})
// 				assert.NoError(t, err)
// 				assert.Equal(t, response.Chains, []exported.ChainName{scalarnet.Scalarnet.Name, evm.Ethereum.Name})
// 			}),
