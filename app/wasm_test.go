...
func TestICSMiddleWare(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run("wasm_enabled:"+testCase.wasm+"-hooks_enabled:"+testCase.hooks, func(t *testing.T) {
			...
			ics4Wrapper := app.InitICS4Wrapper(scalarApp.Keepers, wasmHooks)
...
func TestMaxSizeOverrideForClient(t *testing.T) {
	...
	// reset the sender, because the encoding has changed after calling the root cmd from prefix 'cosmos' to 'scalar'
