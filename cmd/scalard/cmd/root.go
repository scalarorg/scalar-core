...
// NewRootCmd creates a new root command for scalard. It is called once in the
// main function.
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	rootCmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, _ []string) error {
			...
			scalarAppTemplate, scalarAppConfig := initAppConfig()
			err := server.InterceptConfigsPreRunHandler(cmd, scalarAppTemplate, scalarAppConfig)
			if err != nil {
				return err
			}
...
func initAppConfig() (string, interface{}) {
	...
	scalarAppConfig := ScalarAppConfig{
		Config: *srvCfg,
	}

	customAppTemplate := serverconfig.DefaultConfigTemplate

	return customAppTemplate, scalarAppConfig
}
...
func export(encCfg params.EncodingConfig) servertypes.AppExporter {
	return func(logger log.Logger, db dbm.DB, traceStore io.Writer, height int64, forZeroHeight bool, jailAllowedAddrs []string,
		...
		aApp := app.NewScalarApp(logger, db, traceStore, height == -1, map[int64]bool{}, homePath, wasmdir,
